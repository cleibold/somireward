...
bins=12
bin_start,bin_end=[],[]
for n in range(-binwidth*bins,binwidth*bins,binwidth):
    bin_start.append(n)
...
def get_average_spike_bins(bin_start,bin_end,rew_times):
    ...
    binned_spikes=N.empty((unit_counter,len(rew_times)*bins*2))
...
def run_analysis_master(data_dir,random_iterations=False):
    ...
    for n in range(len(dir_list)):
        ...
        X_raw=N.empty((4,len(speed_binned)))
        X_raw[0]=speed_binned
        X_raw[1]=licks_binned
        X_raw[2]=reward_binned
        X_raw[3]=acc_binned
        ...
        for i in range(len(firing_binned)):
            ...
            check3=N.ravel(N.isfinite(X_raw[2]))
            check4=N.ravel(N.isfinite(X_raw[3]))
            check5=N.ravel(N.isfinite(y_raw))
            
            ind=[]
            for k in range(len(check3)):
                if check1[k]==True and check2[k]==True and check3[k]==True and check4[k]==True and check5[k]==True:
                    ind.append(k)
            ind=N.asarray(ind)
            X=X_raw[:,ind]
            y=y_raw[ind]        
            ...
            for k in range(iterations):
                ...
                X_shift[3]=N.random.permutation(X[3])
            ...
            if score>N.percentile(score_temp,sig_thres):
                ...
                if random_iterations==True:
                    ...
                    for k in range(iterations):
                        ...
                        score_temp.append(score_shift)
                    score_temp=N.nanmean(score_temp)
                    red_rew.append(score-score_temp)
    ...
    res={'score':N.asarray(score_total),'coef':coeffs_total,'coef legend':['speed','licks','reward', 'acceleration'],'cell ids':cell_names_total,
          'score wo speed':N.asarray(red_speed),'score wo lick':N.asarray(red_lick),'score wo reward':N.asarray(red_rew),'score wo acc':N.asarray(red_acc),
          'X':Xs,'y':ys,
          'n sig':n_sig_modulated,'n non sig':n_unmodulated}
...
used_test.append(used) 
t,p,used=stat_comp(res['high']['score'],res['high']['score wo reward'])
tt.append(t)
pp.append(p)
comp.append('high v wo reward')
used_test.append(used) 
...
used_test.append(used) 
t,p,used=stat_comp(res['high']['score wo reward'],res['low']['score wo reward'])
tt.append(t)
pp.append(p)
comp.append('high v low wo reward')
used_test.append(used) 
...
res['params']=params        
os.chdir(target_dir)
N.save("linear_modelling_of_SOMI_activity_during_reward.npy",res)                     